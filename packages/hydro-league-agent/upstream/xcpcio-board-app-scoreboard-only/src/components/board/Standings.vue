...
const officialRankByTeamId = computed(() => {
  const result = new Map<string, number | null>();
  let officialPosition = 1;
  let previousOfficial: Team | undefined;
  let previousRank = 0;
  for (const team of teams.value) {
    if (team.isUnofficial) {
      result.set(team.id, null);
      continue;
    }
    const displayedRank = previousOfficial?.isEqualRank(team)
      ? previousRank
      : officialPosition;
    result.set(team.id, displayedRank);
    previousOfficial = team;
    previousRank = displayedRank;
    officialPosition += 1;
  }
  return result;
});
        ...
        <tbody>
          ...
          >
            ...
            >
              <TeamUI
                ...
                :rank="rank"
                :team="giantTeam.team"
                :display-rank="officialRankByTeamId.get(giantTeam.team.id) ?? null"
          ...
          >
            <TeamUI
              ...
              :rank="rank"
              :team="team"
              :display-rank="officialRankByTeamId.get(team.id) ?? null"
              :is-filter="true"
            />
          </template>
