...
import type { BoardData } from "@board/composables/useQueryBoardData";
...
const leagueStatus = ref<BoardData["league_status"]>();
...
watch(data, async () => {
  ...
  leagueStatus.value = data.value?.league_status;
...
<template>
  <div>
    ...
    >
      <div
        v-if="leagueStatus && !leagueStatus.complete && leagueStatus.message"
        :class="[widthClass]"
        flex justify-center
        mb-4
      >
        <div
          class="w-[92%] border-l-4 border-red-700 bg-red-50 px-4 py-3 text-red-900 dark:bg-red-950 dark:text-red-100"
          role="status"
        >
